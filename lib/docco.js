(function() {
  var destination, docco_styles, docco_template, ensure_directory, escaper, exec, ext, fs, generate_documentation, generate_html, get_language, highlight, highlight_end, highlight_start, l, languages, parse, path, showdown, sources, spawn, template, _ref;
  ...
  parse = function(source, code) {
    var code_text, docs_text, has_code, in_comment_block, language, line, lines, save, sections, _i, _len;
    lines = code.split('\n');
    sections = [];
    ...
    for (_i = 0, _len = lines.length; _i < _len; _i++) {
      ...
      } else if (language.comment_begin_matcher && line.match(language.comment_begin_matcher)) {
  ...
  for (ext in languages) {
    l = languages[ext];
    escaper = function(exp) {
      return exp.replace(/(\/|\*|\\)/g, "\\$1");
    };
    l.comment_matcher = new RegExp('^\\s*' + l.symbol + '\\s?');
    if (l.begin) {
      l.comment_begin_matcher = new RegExp('^\\s*' + escaper(l.begin) + '\\s?');
    }
    if (l.middle) {
      l.comment_middle_matcher = new RegExp('^\\s*' + escaper(l.middle) + '\\s?');
    }
    if (l.end) {
      l.comment_end_matcher = new RegExp('^\\s*' + escaper(l.end) + '\\s?');
    }
